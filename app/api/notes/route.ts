...
export async function POST(request: NextRequest) {
  ...
  const { project_id, interview_id, content } = await request.json()
  ...
  const result = await query(
    `INSERT INTO notes (project_id, interview_id, content, created_by) VALUES ($1,$2,$3,$4) RETURNING *`,
    [project_id, interview_id || null, content.trim(), user.user_id]
  )
