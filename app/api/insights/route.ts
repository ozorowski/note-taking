...
export async function POST(request: NextRequest) {
  ...
  const { project_id, content, evidence_summary } = await request.json()
  ...
  const result = await query(
    `INSERT INTO insights (project_id, content, evidence_summary, created_by) VALUES ($1,$2,$3,$4) RETURNING *`,
    [project_id, content.trim(), evidence_summary?.trim() || null, user.user_id]
  )
