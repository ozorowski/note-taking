...
export async function POST(request: NextRequest) {
  ...
  const result = await query(
    `INSERT INTO interviews (project_id, participant_name, raw_notes, created_by) VALUES ($1,$2,$3,$4) RETURNING *`,
    [project_id, participant_name.trim(), raw_notes?.trim() || null, user.user_id]
  )
